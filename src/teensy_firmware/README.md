...
| 폴더 | 내용 |
| --- | --- |
| [`arm_microros_teensy/`](arm_microros_teensy/arm_microros_teensy.ino) | **최종 통합 펌웨어.** 2층 제어(ESO 외란 관측기 + 태스크 공간 능동 서스펜션), MIT 임피던스 홀드, micro-ROS 논블로킹 재접속, 열 보호, IMU 섹터 스냅 호밍, 중력 보상 구조까지 포함한 실제 동작 버전. |
...
- **1층 — ESO 외란 관측기** (손목 pitch): AK40에는 적용하지 못했다. MIT 프로토콜이 전류값을
  회신하지 않아 구동 토크와 외란 토크를 분리할 수 없기 때문이다. 롤 축은 구현했으나 발산이
  확인되어 비활성화했다.
- **2층 — 태스크 공간 능동 서스펜션** (어깨+팔꿈치): 컵 IMU 가속도를 대역통과(0.8–8 Hz)해
  평면 내 팁 변위를 추정하고, damped 2R 자코비안 의사역행렬로 관절 보정량으로 변환해 MIT 홀드
  설정점 위에 더한다. 소스의 `Active suspension` 주석 블록에 설계 근거와 튜닝 이력이 있다.
